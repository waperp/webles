...
function detectmob() {
    if (
        ...
        $(
            "ul.nav-alignment-dynamic li:not(.elementskit-dropdown-has) > a"
        ).click(function() {
            // refreshPage();
...
$(document).ready(function() {
    ...
    $("#descripcion-general-div").append(
        '<a class="btn-vermas" style="color: #49c7ed;font-size:15px">Ver más</a>'
    );
...
function formatState1(state) {
    ...
    var $state = $(
        '<span> <i style="color:black; font-size: 1.2em;" class="fa fa-2x ' +
            state.id +
            '"></i> <strong> ' +
            state.text +
            "</strong></span>"
...
function myFunction() {
    ...
    document.getElementById("jinu").src =
        ...
        x +
        "&layout=button_count&size=large&mobile_iframe=true&width=83&height=28&appId=665910193938787";
}
...
function edit_menu_principal(secusricode) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $("#select2-edit-user-subform").append(
                '<option value="' +
                    data.contypscode +
                    '">' +
                    data.contyptdesc +
            ...
            $("#edit-user-hurempvimgh")
                .parent()
                .css(
                    "background-image",
                    "url('images/" + data.hurempvimgh + "')"
                );
            $("#edit-user-hurempvimgh")
                .parent()
...
function lista_servicio_select(data) {
    ...
    data.forEach(servicesItem => {
        item +=
            ...
            servicesItem.confrmscode +
            ' featured-block style-two col-lg-4 col-md-6 col-sm-12">' +
            '<div class="inner-box wow fadeInLeft visible" data-wow-delay="0ms" data-wow-duration="1500ms">' +
            '<div class="image-layer" style="background-image:url(images/' +
            servicesItem.confrsvbigi +
            ');background-size:cover;background-position: center center "></div>' +
            '<div class="icon-box"><img style="width: 100px; height: 100px;" src="images/' +
            servicesItem.confrsvbigi +
            '"/></div>' +
            '<h3 class="text-center"><a href="#">' +
            servicesItem.confrsttitl +
            "</a></h3>" +
            '<p class="text-center">' +
            servicesItem.confrstdesc.substring(0, 20) +
            "</p>" +
            '<div class="link-box wow fadeInUp  text-center mt-2" data-wow-delay="1000ms">' +
            '<a href="/servicio/' +
            convertToSlug(servicesItem.confrsttitl) +
            "/" +
            servicesItem.secconnuuid +
            '" class="theme-btn btn-style-two other"><i>Ver Más</i> <spanclass="arrow icon icon-arrow_right"></span></a></div> ' +
            "</div>" +
            "</div>";
    });
